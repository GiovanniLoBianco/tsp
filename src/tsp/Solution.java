 ...
 * 
 * The attribute #m_nbCities is the number of cities in the problem. The array
 * #m_cities has size m_nbCities+1 and it should start with the first city for
 * the route and end with the same city.
 * 
 * For example, suppose a problem with 5 customers (hence, the corresponding
 * cities index range from 0 to 4). The following array represents a TSP
 ...
 * m_cities[0]=m_cities[m_nbCities] - Apart from the city at indices 0 and
 * #m_nbCities, each customer index should be represented once and only one in
 * #m_cities.
 * 
 * ** Creating, accessing or modifying a solution:**
 * 
 * A Solution object is created for a given TSP problem, represented by an
 * Instance object. Let instance be an object of class Instance, the following
 * code creates a solution for this problem:
 * 
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Solution mySolution = new Solution(instance);
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * 
 * During its creation, this object is initialized with city 0 at all position.
 * It should be modified to represent a feasible solution. This modification is
 * done by the method {@link #setCityPosition(int city, int position)}, which
 * sets city s at position in the solution. For example, the following code
 * creates a TSP solution that visits all customers in increasing city order:
 * 
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~ for(int i=0; i < instance.getNbCities(); i++) {
 * mySolution.setVertexPosition(i, i); } mySolution.setVertexPosition(0,
 * instance.getNbCities()); ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * 
 * To explore a {@link Solution} object, the city at position can be obtained
 * calling {@link #getCity(int position)}.
 * 
 ...
 * {@link #setObjectiveValue(long newValue)}.
 * 
 * 
...
public class Solution {
	 ...
	 * Note that if there is already a city index at position then this city is
	 * replaced by indexCity.
	 * 
	 * @param indexCity
	 *            index of the city to insert.
	 * @param position
	 *            insertion position.
	 * @throws Exception
	 *             returns an error if position is not a valid insertion
	 *             position or indexCity is not a valid city number.
	 */
	public void setCityPosition(int indexCity, int position) throws Exception {
		...
		if ((indexCity < 0) || (indexCity >= m_nbCities))
			throw new Exception("Error Instance.setVertexPosition(i,s) : city value s=" + indexCity
					+ ", must range between 0 and " + (m_nbCities - 1));
	 ...
	 * #m_cities[0]=#m_cities[#m_nbCities] - apart from the city at index 0 and
	 * #m_nbCities, each customer index should be represented once and only one
	 * in #m_cities.
	 * 
	...
	public boolean isFeasible() throws Exception {
		...
		if (m_cities[0] != m_cities[m_nbCities]) {
			...
			m_error += "The first city is " + m_cities[0] + " and the last city is " + m_cities[m_nbCities] + ".\n";
			result = false;
		}
		...
		for (int i = 1; i < m_nbCities; i++) {
			if (occurences[i] != 1) {
				m_error += "Error : the city " + (i) + " is visited " + occurences[i] + " times.\n";
				result = false;
			}
		}
	...
	/**
	 * Print the solution on output `out`. The solution is printed using cities
	 * numbers and not labels.
	 * 
	 * For example if you want to print a solution `mysol` on the screen, use :
	 * 
	...
	/**
	 * @return The objective value of the function. This value should be updated
	 *         when the solution is modified, either calling {@link evaluate}
	 *         which recomputes the cost or {@link setObjectiveValue}
	 */
	...
	/**
	 * @return return a link to the data of the problem associated with the
	 *         solution.
	 */
	...
	public int getCity(int position) throws Exception {
		if ((position < 0) || (position > m_nbCities))
			throw new Exception("Error Instance.getSolution : index " + position
					+ " is not valid, it should range between 0 and " + m_nbCities);
